...
int main() {
    ...
    printf("\ninsert %d\n", inserts[0]);
    root = BST_insert(root, inserts[0], &comps, &reads, &w);
    BST_print(root);
    for (int i = 1; i < RANDOM_OPERATIONS; i++) {
        printf("\ninsert %d\n", inserts[i]);
        BST_insert(root, inserts[i], &comps, &reads, &w);
        BST_print(root);
    }
    ...
    for (int i = 0; i < RANDOM_OPERATIONS; i++) {
        ...
        if (BST_delete(root, deletes[i], &comps, &reads, &w) == 1)
            root = NULL;
        BST_print(root);
    }
