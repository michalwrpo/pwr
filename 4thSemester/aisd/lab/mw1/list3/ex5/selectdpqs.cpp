...
long SortPartition(long* arr, long low, long high, long* pl, long pr, long* swaps, long* comparisons, int depth, bool print) {
    if (arr[low] > arr[high]) {
        swap(swaps, arr, low, high);
    }
    
    // p - left pivot, q - right pivot
    long p = arr[*pl], q = arr[pr];

    swap(swaps, arr, *pl, low);
    swap(swaps, arr, pr, high);
...
long Partition(long* arr, long low, long high, long pivotIndex, long* swaps, long* comparisons) {
...
long Select(long* arr, long low, long high, long goal, long* swaps, long* comparisons) {
    ...
    long r = Partition(arr, low, high, M, swaps, comparisons);
    long index = r - low + 1;
...
void DPQS(long* arr, long low, long high, long* swaps, long* comparisons, bool print, int depth) {
    ...
    long pl = Select(arr, low, high, (high - low + 2) / 3, swaps, comparisons);
    long pr = Select(arr, low, high, 2 * (high - low + 2) / 3, swaps, comparisons);
    pr = SortPartition(arr, low, high, &pl, pr, swaps, comparisons, depth, print);
