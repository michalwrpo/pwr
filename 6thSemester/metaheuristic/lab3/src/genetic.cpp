...
static void mutate(Permutation& p, std::mt19937& gen, std::uniform_int_distribution<size_t>& vertex) {
    ...
    if ((v1 == 0 && v2 >= p.n - 2) || (v1 <= 1 && v2 == p.n - 1)) return;
...
static void partially_mapped_crossover(const Permutation& p1, const Permutation& p2,
                                size_t from, size_t to, std::vector<bool> added, 
                                Permutation& child, std::vector<size_t>& p2_pos) {
    if (from > to) std::swap(from, to);
    ...
    for (size_t j = from; j <= to; ++j) {
        child.elements[j] = p1.elements[j];
        added[child.elements[j]] = true;
    }

    for (size_t j = 0; j < n; ++j) {
        p2_pos[p2.elements[j]] = j;
    }

    for (size_t j = from; j <= to; ++j) {
        size_t candidate{ p2.elements[j] };
        if (added[candidate]) continue;

        size_t idx{ p2_pos[child.elements[j]] };

        while (true) {
            if (child.elements[idx] == n) {
                added[candidate] = true;
                child.elements[idx] = candidate;
                break;
            } else {
                idx = p2_pos[child.elements[idx]];
            }
        }
    }
    ...
    for (size_t j = 0; j < n; ++j) {
        if (added[p2.elements[j]]) continue;

        while (child.elements[i] != n) ++i;
        child.elements[i] = p2.elements[j];
        ++i;
    }
    ...
    while (last_changed < NO_IMPROVEMENT_ITERATIONS) {
        ...
        for (size_t i{ 0 }; i < population_size / 2; ++i) {
            ...
            partially_mapped_crossover(
                population[parent1], population[parent2], vertex(gen), vertex(gen), 
                crossover_mask, next_population[c1_idx], p2_pos
            );
            ...
            size_t c2_idx{ i * 2 + 1};
            partially_mapped_crossover(
                population[parent2], population[parent1], vertex(gen), vertex(gen), 
                crossover_mask, next_population[c2_idx], p2_pos
            );
...
std::pair<Permutation, int> genetic_meme(Graph& g, size_t population_size, size_t parent_pool, double mutation_chance) {
    ...
    while (last_changed < NO_IMPROVEMENT_ITERATIONS) {
        ...
        for (size_t i{ 0 }; i < population_size; ++i) {
            taboo_search(population[i], 10, static_cast<size_t>(10 * steps), taboo, vertex, gen);
