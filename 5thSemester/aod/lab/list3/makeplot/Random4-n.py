...
for fname in sorted(os.listdir(OUTPUT_DIR)):
    # match: Random4-n-{d|l|r}.i.0.ss.res
    m = re.match(r"Random4-n-([dlr])\.(\d+)\.0\.ss\.res$", fname)
    if not m:
        continue
...
for fname in sorted(os.listdir(OUTPUT_DIR)):
    # match: Random4-n-{d|l|r}.i.1.ss.res
    m = re.match(r"Random4-n-([dlr])\.(\d+)\.1\.ss\.res$", fname)
    if not m:
        continue
