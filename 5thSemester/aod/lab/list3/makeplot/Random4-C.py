...
for fname in sorted(os.listdir(OUTPUT_DIR)):
    # match: Random4-C-{d|l|r}.i.0.ss.res
    m = re.match(r"Random4-C-([dlr])\.(\d+)\.0\.ss\.res$", fname)
    if not m:
        continue
...
for fname in sorted(os.listdir(OUTPUT_DIR)):
    # match: Random4-C-{d|l|r}.i.1.ss.res
    m = re.match(r"Random4-C-([dlr])\.(\d+)\.1\.ss\.res$", fname)
    if not m:
        continue
